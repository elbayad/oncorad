...
// ── Radio-map persistence helper ──
const saveRadioMapToDisk = (map) => {
    try {
        fs.writeFileSync(FP_DB_FILE, JSON.stringify(map, null, 2));
    } catch (e) {
        console.error('[RTLS] Failed to save radio map:', e.message);
    }
};
...
class RtlsService extends EventEmitter {
    constructor() {
        ...
        this.radioMap = this.loadRadioMap();
    ...
    // ═══════════════════════════════════════════════════════════════════════
    //  3) COMPUTE RAW POSITION (hybrid FP + WCL)
    // ═══════════════════════════════════════════════════════════════════════
    computeRawPosition(floor, vector) {
        ...
        // WCL is now floor-filtered
        const wclPos = this.calculateWCLPosition(vector, floor);

        // Fingerprinting (Quick Win: k=5, weighted)
        const fpPos = USE_FINGERPRINT ? this.calculateFingerprintPosition(floor, floorVector) : null;

        // Adaptive Hybrid Mix (Quick Win)
        if (fpPos && wclPos) {
            let fpWeight = HYBRID_MIX; // default 0.35 or from config

            // Confidence check: distance to nearest fingerprint point
            // fpPos now returns confidence metrics
            const confidence = fpPos.confidence || 0.5;

            if (confidence > HYBRID_CONF_THRESHOLD) {
                fpWeight = 0.8; // High confidence in FP
            } else if (anchorCount < HYBRID_MIN_ANCHORS) {
                fpWeight = 0.3; // Low anchor count, favor WCL (which is 1 - fpWeight = 0.7)
            }

            return {
                x: fpPos.x * fpWeight + wclPos.x * (1 - fpWeight),
                y: fpPos.y * fpWeight + wclPos.y * (1 - fpWeight),
                fpWeight,
                anchorCount
            };
        }
    ...
    calculateFingerprintPosition(floor, vector) {
        const points = this.radioMap[floor] || [];
        if (points.length === 0) return null;

        const dists = points.map(p => ({
            p: p,
            d: this.euclideanDistance(vector, p.rssi)
        }));
        dists.sort((a, b) => a.d - b.d);

        const k = Math.min(FP_K, dists.length);
        if (k === 0) return null;

        // Weighted Average: weight = 1 / distance^2 (Quick Win)
        let sumX = 0, sumY = 0, sumW = 0;
        for (let i = 0; i < k; i++) {
            const d = Math.max(dists[i].d, 0.1); // Avoid div by zero
            const w = 1 / (d * d);
            sumX += dists[i].p.x * w;
            sumY += dists[i].p.y * w;
            sumW += w;
        ...
        const bestDist = dists[0].d;
        // Confidence: higher if best candidate is very close (e.g., bestDist < 2.0)
        const confidence = 1 / (1 + bestDist / 5.0);
    ...
    calculateFloorScore(floorKey, vector) {
        const points = this.radioMap[floorKey] || [];
        if (points.length === 0) return 9999;
        const dists = points.map(p => this.euclideanDistance(vector, p.rssi));
        dists.sort((a, b) => a - b);
        const k = Math.min(FP_K, dists.length);
        let sum = 0;
    ...
    scoreRoomsByRadio(floor, vector) {
        ...
        for (const room of floorRooms) {
            ...
            const gateNorm = sRoom === null ? 0 : Math.max(0, Math.min(1, (delta + 5) / 20));
            const rssiNorm = sRoom === null ? 0 : Math.max(0, Math.min(1, (sRoom + 95) / 25));
            const score = 0.55 * conf + 0.30 * gateNorm + 0.15 * rssiNorm;
    ...
    getSemanticZoneEvidence(floor, vector, state) {
        ...
        const enterEvidence = best
            ? (0.55 * best.score) + (0.25 * Math.max(0, Math.min(1, margin / 0.25))) + (0.20 * (best.sRoom !== null && best.sRoom >= ENTER_RSSI ? 1 : 0))
            : 0;
        ...
        if (state.semantic?.mode === 'IN_ROOM' && state.semantic.roomId) {
            ...
            if (current) {
                const weakCurrent = current.score < 0.28 ? 1 : 0;
                ...
                exitEvidence = 0.45 * corridorScore + 0.25 * weakCurrent + 0.15 * weakSignal + 0.15 * outsideDominant;
            }
        }
    ...
    determineSemanticState(tagMac, floor, rawPos, vector, state) {
        ...
        const previous = state.semantic || { mode: 'UNKNOWN', roomId: null, since: Date.now(), confidence: 0 };
        const next = {
            mode: previous.mode,
        ...
        const ENTER_EVIDENCE_MIN = 0.52;
        const ENTER_MARGIN_MIN = 0.08;
        const CANDIDATE_ENTER_CYCLES = Math.max(2, ENTER_CYCLES);
        const EXIT_EVIDENCE_MIN = 0.70;
        const EXIT_CONFIRM_CYCLES = Math.max(EXIT_CYCLES, 6);

        const best = evidence.best;
        const bestRoomId = best?.room?.id ?? null;
        const strongEnter = !!best && evidence.enterEvidence >= ENTER_EVIDENCE_MIN && evidence.margin >= ENTER_MARGIN_MIN;

        if (previous.mode === 'IN_ROOM' && previous.roomId) {
            const current = evidence.ranked.find(x => x.room.id === previous.roomId);
            if (current) {
                next.confidence = Math.max(0.35, Math.min(0.99, 0.85 * previous.confidence + 0.15 * current.score));
            } else {
                next.confidence = Math.max(0.25, previous.confidence * 0.92);
            }
            ...
            if (bestRoomId && bestRoomId !== previous.roomId && strongEnter && evidence.margin >= 0.16) {
                next.mode = 'IN_ROOM';
                next.roomId = previous.roomId;
                next.reason = 'hold_current_room';
                return next;
            }
        ...
        if (previous.mode === 'EXIT_CANDIDATE' && previous.roomId) {
            if (evidence.exitEvidence >= EXIT_EVIDENCE_MIN) {
                ...
                if (next.exitCount >= EXIT_CONFIRM_CYCLES) {
                    next.mode = 'CORRIDOR';
                    next.roomId = null;
                    next.confidence = Math.max(0.4, evidence.corridorScore);
        ...
        next.roomId = null;
        next.reason = next.mode === 'CORRIDOR' ? 'corridor_score' : 'unknown_hold';
        return next;
    }

    renderSemanticPosition(floor, rawPos, semantic, previousPos) {
        if (semantic.mode === 'IN_ROOM' || semantic.mode === 'EXIT_CANDIDATE' || semantic.mode === 'ROOM_CANDIDATE') {
            const roomId = semantic.roomId || semantic.candidateRoomId;
    ...
    async updateTagState(tagMac, floor, vector) {
        ...
        try {
            ...
            dbg(`[PIPELINE:SEM] tag=${tagMac.slice(-4)} floor=${floor} semantic=${semantic.mode} ` +
                `room=${semantic.roomId || '-'} reason=${semantic.reason} conf=${(semantic.confidence || 0).toFixed(2)} ` +
