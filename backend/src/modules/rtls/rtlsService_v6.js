...
import Floor6RssiLab from './test_etage6_rssi.js';
import {
    BUCKET_MS,
    FLOOR_LOCK_SEC,
    FLOOR_MARGIN,
...
class RtlsService extends EventEmitter {
    constructor() {
        ...
        this.loadRooms().then(() => {
            ...
            // Bind lab data
            this.floor6Lab.bindStaticData({
                anchors: this.anchors,
                rooms: this.rooms,
                anchorRoomMap: this.anchorRoomMap
            });
        ...
        // Floor 6 Lab
        this.floor6Lab = new Floor6RssiLab();
        // Start session for the Bloc6 tag
        this.floor6Lab.startSession({
            tagMac: '18690a7612e0',
            focusRooms: ['Bloc5', 'Bloc6'],
            label: 'bloc6_diagnostic'
        });
    }
    ...
    computeRoomAnchorProximity(room, floor, vector) {
        ...
        for (const [macRaw, rssi] of Object.entries(vector)) {
            ...
            if (positiveMacs.has(mac)) positiveWeight += w;
    ...
    determineSemanticState(tagMac, floor, rawPos, vector, state) {
        ...
        const ROOM_SWITCH_MARGIN_MIN = 0.10;
        const ROOM_SWITCH_GAIN_MIN = 0.08;
        const ROOM_SWITCH_EVIDENCE_MIN = 0.60;
        const ROOM_SWITCH_CYCLES = 3;
        ...
        if (previous.mode === 'IN_ROOM' && previous.roomId) {
            ...
            const otherClearlyDominant =
                ...
                !!best &&
                !evidence.ambiguousTop2 &&
                evidence.margin >= ROOM_SWITCH_MARGIN_MIN &&
                best.score >= currentScore + ROOM_SWITCH_GAIN_MIN &&
                evidence.enterEvidence >= ROOM_SWITCH_EVIDENCE_MIN &&
                (best.proximityScore ?? 0) >= ((current?.proximityScore ?? 0) + 0.06);
            ...
            if (otherClearlyDominant) {
                ...
                if (next.candidateCount >= ROOM_SWITCH_CYCLES) {
                    next.mode = 'IN_ROOM';
    ...
    renderSemanticPosition(tagMac, floor, rawPos, semantic, previousPos) {
        ...
        if (semantic.mode === 'ROOM_SWITCH_CANDIDATE') {
            ...
            const pos = fromRoom ? this.getRoomCentroid(fromRoom) : this.applySmoothing(rawPos, previousPos);
            return {
                pos,
    ...
    async updateTagState(tagMac, floor, vector) {
        ...
        try {
            ...
            // Floor 6 Lab Ingestion
            if (tagMac === '18690a7612e0') {
            }
            this.floor6Lab.ingestVector({
                tagMac,
                floor,
                vector,
                rawPos,
                semanticState: state.semantic,
                lockedRoomId: state.roomLock?.roomId ?? null,
                displayZoneName: detectedZoneName
            });
        } catch (error) {
