...
import RoomProfilerAgent from './room-profiler.service.js';
...
// ── V5: radiomap disabled on purpose ──
...
class RtlsService extends EventEmitter {
    constructor() {
        ...
        this.radioMap = {}; // V5: radiomap intentionally disabled
        ...
        this.anchorTuning = new Map(); // floor:room:anchor -> tuning row
        this.switchTuning = new Map(); // floor:from:to -> tuning row
        this.assetAssignments = new Map(); // assetMac → roomId
        ...
        this.loadRooms().then(() => {
            ...
            // Snap assets to their assigned rooms at startup
            this.snapToRoom();
        });
        this.loadMetadata();
        this.loadSemanticTuning().catch(e => console.error('[RTLS] Error loading semantic tuning:', e.message));
        ...
        // Room Profiler
        this.roomProfiler = new RoomProfilerAgent();

        // Refresh assignments every 5 minutes
        setInterval(() => this.loadAssetAssignments(), 300000);
        this.loadAssetAssignments(); // Initial load
    }

    async loadAssetAssignments() {
        try {
            const res = await pool.query("SELECT id, room_id FROM assets WHERE room_id IS NOT NULL");
            this.assetAssignments.clear();
            res.rows.forEach(a => {
                this.assetAssignments.set(a.id.toLowerCase(), a.room_id);
            });
            dbg(`[RTLS] Loaded ${this.assetAssignments.size} asset room assignments.`);
        } catch (e) {
            console.error('[RTLS] Error loading asset assignments:', e.message);
        }
    }
    ...
    async loadRooms() {
        try {
            const roomsRes = await pool.query(
                `SELECT id, room_number, room_number as name, floor_id,
            ...
            const zonesRes = await pool.query("SELECT id, name, floor_id, ST_AsGeoJSON(polygon) as polygon_json FROM zones WHERE polygon IS NOT NULL");
            const zones = zonesRes.rows.map(z => ({
                ...z,
                type: 'zone',
                polygon: z.polygon_json ? JSON.parse(z.polygon_json) : null
            }));
    ...
    buildAnchorRoomMap() {
        ...
        for (const [mac, room] of this.anchorRoomMap) {
            ...
            if (a && String(a.floor_id) === '6') {
                ...
                f6Entries.push(`${isPrimary} ${a.name} → ${this.getRoomLabel(room)}`);
            }
        }
    }
    ...
    flushBucket(tagMac) {
        ...
        // 2.5 Hook: Room Profiler
        if (this.roomProfiler) {
            this.roomProfiler.ingestVector(tagMac, filteredVector);
        }
    ...
    doorGatingSchmitt(state, floor, vector) {
        ...
        for (const room of floorRooms) {
            ...
            dbg(`[SCHMITT] room=${this.getRoomLabel(room)} S_room=${sRoom?.toFixed(1)} S_out=${sOut?.toFixed(1)} ` +
        ...
        try {
            const anchorSql = `
                SELECT
                    ...
                    force_mapped_room,
                    enabled
                FROM public.rtls_room_anchor_tuning
                WHERE enabled = TRUE
            `;
            ...
            const switchSql = `
                SELECT
                    ...
                    required_cycles,
                    enabled
                FROM public.rtls_room_switch_tuning
                WHERE enabled = TRUE
            `;

            const anchorRes = await pool.query(anchorSql);
            const switchRes = await pool.query(switchSql);

            this.anchorTuning = new Map();
            for (const row of anchorRes.rows || []) {
                const floorId = Number(row.floor_id);
                const roomId = Number(row.room_id);
                const anchorMac = String(row.anchor_mac).toLowerCase();
                const key = `${floorId}:${roomId}:${anchorMac}`;
                this.anchorTuning.set(key, {
                    floor_id: floorId,
                    room_id: roomId,
                    anchor_mac: anchorMac,
                    proximity_bonus: Number(row.proximity_bonus || 0),
                    score_bonus: Number(row.score_bonus || 0),
                    ownership_bonus: Number(row.ownership_bonus || 0),
                    primary_bonus: Number(row.primary_bonus || 0),
                    bleed_penalty: Number(row.bleed_penalty || 0),
                    force_mapped_room: Boolean(row.force_mapped_room),
                    enabled: Boolean(row.enabled)
                });
            }

            this.switchTuning = new Map();
            for (const row of switchRes.rows || []) {
                const floorId = Number(row.floor_id);
                const fromRoomId = Number(row.from_room_id);
                const toRoomId = Number(row.to_room_id);
                const key = `${floorId}:${fromRoomId}:${toRoomId}`;
                this.switchTuning.set(key, {
                    floor_id: floorId,
                    from_room_id: fromRoomId,
                    to_room_id: toRoomId,
                    min_score_gain: Number(row.min_score_gain || 0.08),
                    min_proximity_gain: Number(row.min_proximity_gain || 0.05),
                    min_margin_top2: Number(row.min_margin_top2 || 0.10),
                    min_evidence: Number(row.min_evidence || 0.60),
                    required_cycles: Number(row.required_cycles || 3),
                    enabled: Boolean(row.enabled)
                });
            }

        } catch (err) {
            console.error('[SEM_TUNING] load error:', err.message || err);
            this.anchorTuning = new Map();
            this.switchTuning = new Map();
        }
    }

    getAnchorTuning(floorId, roomId, anchorMac) {
        const key = `${Number(floorId)}:${Number(roomId)}:${String(anchorMac).toLowerCase()}`;
        return this.anchorTuning.get(key) || null;
    }

    getSwitchTuning(floorId, fromRoomId, toRoomId) {
        const key = `${Number(floorId)}:${Number(fromRoomId)}:${Number(toRoomId)}`;
        return this.switchTuning.get(key) || null;
    }

    getEffectiveAnchorMappedRoom(floorId, roomId, anchorMac, defaultMappedRoomId = null) {
        const tuning = this.getAnchorTuning(floorId, roomId, anchorMac);
        if (tuning?.force_mapped_room) {
            return String(roomId);
        }
        return defaultMappedRoomId != null ? String(defaultMappedRoomId) : null;
    }

    getRoomLabel(room) {
        if (!room) return 'unknown';
        return room.room_number || room.name || String(room.id);
    }

    // ═══════════════════════════════════════════════════════════════════════
    //  5b) SEMANTIC ROOM EVIDENCE / STATE MACHINE
    // ═══════════════════════════════════════════════════════════════════════
    getRoomCentroid(room, tagId = null) {
        let x = parseFloat(room.centroid_x ?? room.anchor_x ?? 0);
        let y = parseFloat(room.centroid_y ?? room.anchor_y ?? 0);

        if (tagId) {
            // Create a deterministic offset based on the last characters of the MAC
            // This avoids tags dancing but keeps them separated
            const hash = parseInt(tagId.slice(-4), 16) || 0;
            const angle = (hash % 360) * (Math.PI / 180);
            const radius = 20 + (hash % 40); // 20-60cm radius
    ...
    _rssiToWeight(rssi) {
        if (typeof rssi !== 'number' || Number.isNaN(rssi) || rssi < MIN_RSSI) return 0;
        return Math.pow(Math.max(0, rssi - MIN_RSSI), Math.max(1, WCL_EXP));
    }

    _applyAnchorRoomTuning(room, floor, vector, base) {
        let scoreBonus = 0;
        ...
        for (const [macRaw, rssi] of Object.entries(vector || {})) {
            const mac = String(macRaw).toLowerCase();
            const tuning = this.getAnchorTuning(String(floor), String(room.id), mac);
            if (!tuning) continue;

            const rssiWeight = Math.max(0, Number(rssi) + 100) / 40;
            scoreBonus += Number(tuning.score_bonus || 0);
            proximityBonus += Number(tuning.proximity_bonus || 0);
            ownershipBonus += Number(tuning.ownership_bonus || 0);
            primaryBonus += Number(tuning.primary_bonus || 0);
            bleedPenalty += Number(tuning.bleed_penalty || 0) * rssiWeight;
        }

        const tuned = { ...base };
        tuned.radioScore = (base.radioScore ?? 0) + scoreBonus - bleedPenalty;
        tuned.proximityScore = (base.proximityScore ?? 0) + proximityBonus;
        tuned.ownershipScore = (base.ownershipScore ?? 0) + ownershipBonus;
        tuned.primaryScore = (base.primaryScore ?? 0) + primaryBonus;
        tuned.scoreBonus = scoreBonus;
        tuned.proximityBonus = proximityBonus;
        tuned.ownershipBonus = ownershipBonus;
        tuned.primaryBonus = primaryBonus;
        tuned.bleedPenalty = bleedPenalty;
        return tuned;
    }


    computeRoomAnchorProximity(room, floor, vector) {
        const roomPos = this.getRoomCentroid(room);
        let weightedInvDist = 0;
        let totalWeight = 0;
        let positiveWeight = 0;
        let negativeWeight = 0;
        let strongestAnchorDist = Infinity;
        let strongestAnchorMac = null;
        const gate = this.roomGates.get(room.id);
        const positiveMacs = new Set((gate?.positive || []).map(x => String(x.mac).toLowerCase()));
        const negativeMacs = new Set((gate?.negative || []).map(x => String(x.mac).toLowerCase()));

        for (const [macRaw, rssi] of Object.entries(vector)) {
            const mac = String(macRaw).toLowerCase();
            const anchor = this.anchors.get(mac);
            if (!anchor || String(anchor.floor_id) !== String(floor)) continue;

            const w = this._rssiToWeight(rssi);
            if (w <= 0) continue;

            const anchorPos = { x: parseFloat(anchor.x), y: parseFloat(anchor.y) };
            const dist = this._distanceCm(anchorPos, roomPos);
            const invDist = 1 / Math.max(100, dist);

            weightedInvDist += w * invDist;
            totalWeight += w;

            const defaultMapped = this.anchorRoomMap.get(mac);
            const effectiveMappedRoomId = this.getEffectiveAnchorMappedRoom(String(floor), String(room.id), mac, defaultMapped?.id || null);

            if (positiveMacs.has(mac) || effectiveMappedRoomId === String(room.id)) positiveWeight += w;
            if (negativeMacs.has(mac)) negativeWeight += w;

            if (w > 0 && dist < strongestAnchorDist) {
                strongestAnchorDist = dist;
                strongestAnchorMac = mac;
            }
        }
        ...
        if (totalWeight <= 0) {
            return {
                ...
                positiveAffinity: 0,
                negativePenalty: 0,
                strongestAnchorDist: Infinity,
                strongestAnchorMac: null
            };
        }

        const rawInvDist = weightedInvDist / totalWeight;
        const distanceNorm = Math.max(0, Math.min(1, rawInvDist * 900));
        const positiveAffinity = totalWeight > 0 ? positiveWeight / totalWeight : 0;
        const negativePenalty = totalWeight > 0 ? negativeWeight / totalWeight : 0;
        const strongestBonus = Number.isFinite(strongestAnchorDist)
            ? Math.max(0, Math.min(1, 1 - (strongestAnchorDist / 1200)))
            : 0;

        const score =
            0.55 * distanceNorm +
            0.25 * positiveAffinity +
            0.10 * strongestBonus -
            0.10 * negativePenalty;
    ...
    scoreRoomTuning(floor, room, vector) {
        ...
        const proximity = this.computeRoomAnchorProximity(room, floor, vector);

        const base = {
            radioScore: 0.42 * conf + 0.18 * gateNorm + 0.12 * rssiNorm - 0.08 * outPenalty,
            proximityScore: 0.36 * proximity.score,
            ownershipScore: 0.18 * proximity.positiveAffinity,
            primaryScore: 0.10 * (Number.isFinite(proximity.strongestAnchorDist)
                ? Math.max(0, Math.min(1, 1 - (proximity.strongestAnchorDist / 1200)))
                : 0)
        };

        const tuned = this._applyAnchorRoomTuning(room, floor, vector, base);
        const score =
            tuned.radioScore +
            tuned.proximityScore +
            tuned.ownershipScore +
            tuned.primaryScore;
        ...
        return {
            ...
            rssiNorm,
            outPenalty,
            proximityScore: tuned.proximityScore,
            proximityRawInvDist: proximity.rawInvDist,
            proximityPositiveAffinity: proximity.positiveAffinity,
            proximityNegativePenalty: proximity.negativePenalty,
            strongestAnchorDist: proximity.strongestAnchorDist,
            strongestAnchorMac: proximity.strongestAnchorMac,
            ownershipScore: tuned.ownershipScore,
            primaryScore: tuned.primaryScore,
            radioScore: tuned.radioScore,
            scoreBonus: tuned.scoreBonus,
            proximityBonus: tuned.proximityBonus,
            ownershipBonus: tuned.ownershipBonus,
            primaryBonus: tuned.primaryBonus,
            bleedPenalty: tuned.bleedPenalty
        };
    }

    scoreRoomsByRadio(floor, vector, tagMac = null) {
        const candidates = [];
        const floorRooms = this.rooms.filter(r => r.type === 'room' && String(r.floor_id) === String(floor));
        const assignedRoomId = tagMac ? this.assetAssignments.get(tagMac.toLowerCase()) : null;
        const MANUAL_ASSIGNMENT_BONUS = 0.25;

        for (const room of floorRooms) {
            const tuned = this.scoreRoomTuning(floor, room, vector);
            let finalScore = tuned.score;

            // Apply Manual Assignment Bonus
            if (assignedRoomId && room.id === assignedRoomId) {
                finalScore += MANUAL_ASSIGNMENT_BONUS;
                dbg(`[SCORING] Applied manual assignment bonus to room ${room.name} for tag ${tagMac}`);
            }
            ...
            candidates.push({
                ...
                sOut: tuned.sOut,
                delta: tuned.delta,
                gateNorm: tuned.gateNorm,
                rssiNorm: tuned.rssiNorm,
                outPenalty: tuned.outPenalty,
                proximityScore: tuned.proximityScore,
                proximityRawInvDist: tuned.proximityRawInvDist,
                proximityPositiveAffinity: tuned.proximityPositiveAffinity,
                proximityNegativePenalty: tuned.proximityNegativePenalty,
                strongestAnchorDist: tuned.strongestAnchorDist,
                strongestAnchorMac: tuned.strongestAnchorMac,
                ownershipScore: tuned.ownershipScore,
                primaryScore: tuned.primaryScore,
                radioScore: tuned.radioScore,
                scoreBonus: tuned.scoreBonus,
    ...
    getSemanticZoneEvidence(floor, vector, state, tagMac = null) {
        const ranked = this.scoreRoomsByRadio(floor, vector, tagMac);
        ...
        const ambiguousTop2 = !!best && !!second && margin < 0.10;
        const enterEvidence = best
            ? (0.42 * best.score) +
            (0.20 * Math.max(0, Math.min(1, margin / 0.30))) +
            (0.12 * Math.max(0, Math.min(1, (top2Ratio - 1) / 0.80))) +
            (0.10 * (best.sRoom !== null && best.sRoom >= ENTER_RSSI ? 1 : 0)) +
            (0.16 * (best.proximityScore ?? 0))
            : 0;
    ...
    getSwitchThresholds(floorId, fromRoomId, toRoomId) {
        const rule = this.getSwitchTuning(String(floorId), String(fromRoomId), String(toRoomId));
        return {
            minScoreGain: Number(rule?.min_score_gain ?? 0.08),
            minProximityGain: Number(rule?.min_proximity_gain ?? 0.05),
            minMarginTop2: Number(rule?.min_margin_top2 ?? 0.10),
            minEvidence: Number(rule?.min_evidence ?? 0.60),
            requiredCycles: Number(rule?.required_cycles ?? 3)
        };
    }

    determineSemanticState(tagMac, floor, rawPos, vector, state) {
        const evidence = this.getSemanticZoneEvidence(floor, vector, state, tagMac);
        const now = Date.now();
        ...
        if (previous.mode === 'IN_ROOM' && previous.roomId) {
            ...
            const bestIsOtherRoom = !!bestRoomId && bestRoomId !== previous.roomId;
            const currentScore = current?.score ?? 0;
            const switchCfg = bestIsOtherRoom ? this.getSwitchThresholds(floor, previous.roomId, bestRoomId) : null;
            const otherClearlyDominant =
                bestIsOtherRoom &&
                !!best &&
                !evidence.ambiguousTop2 &&
                evidence.margin >= (switchCfg?.minMarginTop2 ?? 0.10) &&
                best.score >= currentScore + (switchCfg?.minScoreGain ?? 0.08) &&
                evidence.enterEvidence >= (switchCfg?.minEvidence ?? 0.60) &&
                (best.proximityScore ?? 0) >= ((current?.proximityScore ?? 0) + (switchCfg?.minProximityGain ?? 0.05));

            if (otherClearlyDominant) {
                ...
                if (next.candidateCount >= (switchCfg?.requiredCycles ?? 3)) {
                    next.mode = 'IN_ROOM';
                    next.roomId = bestRoomId;
                    next.since = now;
                    next.confidence = Math.max(best.score, next.confidence);
                    next.exitCount = 0;
                    next.lastStableRoomId = bestRoomId;
                    next.reason = 'room_switch_confirmed';
                    return next;
                }

                next.mode = 'ROOM_SWITCH_CANDIDATE';
                next.roomId = previous.roomId;
                next.lastStableRoomId = previous.roomId;
                next.exitCount = 0;
                next.reason = 'room_switch_candidate';
                return next;
            }
            ...
            next.candidateRoomId = null;
            next.candidateCount = 0;
            next.reason = bestIsOtherRoom ? 'hold_vs_competitor' : 'stay_in_room';
            return next;
        }

        if (previous.mode === 'ROOM_SWITCH_CANDIDATE' && previous.roomId) {
            const current = evidence.ranked.find(x => x.room.id === previous.roomId);
            const currentScore = current?.score ?? 0;
            const switchCfg = previous.candidateRoomId ? this.getSwitchThresholds(floor, previous.roomId, previous.candidateRoomId) : null;
            const stillSwitching =
                !!best &&
                !!bestRoomId &&
                bestRoomId === previous.candidateRoomId &&
                !evidence.ambiguousTop2 &&
                evidence.margin >= (switchCfg?.minMarginTop2 ?? 0.10) &&
                best.score >= currentScore + (switchCfg?.minScoreGain ?? 0.08) &&
                evidence.enterEvidence >= (switchCfg?.minEvidence ?? 0.60) &&
            ...
            if (stillSwitching) {
                next.candidateRoomId = previous.candidateRoomId;
                next.candidateCount = (previous.candidateCount || 0) + 1;

                if (next.candidateCount >= (switchCfg?.requiredCycles ?? 3)) {
                    next.mode = 'IN_ROOM';
                    next.roomId = previous.candidateRoomId;
                    next.since = now;
                    next.confidence = Math.max(best.score, 0.65);
                    next.exitCount = 0;
                    next.lastStableRoomId = previous.candidateRoomId;
                    next.reason = 'room_switch_confirmed';
                    return next;
                }
            ...
            if (evidence.exitEvidence >= EXIT_EVIDENCE_MIN) {
                ...
                next.roomId = previous.roomId;
                next.exitCount = 1;
                next.reason = 'switch_to_exit_candidate';
                return next;
            }
            ...
            next.exitCount = 0;
            next.lastStableRoomId = previous.roomId;
            next.reason = 'room_switch_cancelled';
            return next;
        }
    ...
    renderSemanticPosition(tagMac, floor, rawPos, semantic, previousPos) {
        ...
        if (semantic.mode === 'ZONE') {
            const zone = this.rooms.find(r => r.id === semantic.roomId && r.type === 'zone');
            if (zone) {
                return {
                    pos: rawPos,
        ...
        if (semantic.mode === 'ROOM_SWITCH_CANDIDATE') {
            const fromRoom = this.rooms.find(r => r.id === semantic.roomId);
            const toRoom = this.rooms.find(r => r.id === semantic.candidateRoomId);
            const pos = fromRoom ? this.getRoomCentroid(fromRoom, tagMac) : this.applySmoothing(rawPos, previousPos);
            return {
                pos,
                displayName: toRoom ? `${fromRoom?.name || 'room'}->${toRoom.name}` : (semantic.candidateRoomId ? `switch:${semantic.candidateRoomId}` : 'switch'),
                zoneType: 'room_switch_candidate',
                room: fromRoom || null,
                method: semantic.reason
            };
        }
    ...
    determineZone(tagMac, floor, rawPos, vector, state) {
        ...
        for (const room of this.rooms) {
            if (room.type === 'room' && room.polygonGeo && String(room.floor_id) === String(floor)) {
                if (turf(pt, room.polygonGeo)) {
                    ...
                    // dbg(`[ZONE:F6] No poly match for ${this.getRoomLabel(room)} (${room.polygon})`);
                }
            }
        }
        ...
        // ── 3. ZONE POLYGON ──
        let polygonZone = null;
        for (const zone of this.rooms) {
            if (zone.type === 'zone' && zone.polygonGeo && String(zone.floor_id) === String(floor)) {
                if (turf(pt, zone.polygonGeo)) {
                    polygonZone = zone;
                    break;
                }
            }
        ...
        if (polygonZone) {
            return {
                ...
                snapCentroid: false,
                method: 'zone_polygon',
                displayName: polygonZone.name
            };
        }

        // ── 4. CORRIDOR POLYGON + HYSTERESIS ──
    ...
    async updateTagState(tagMac, floor, vector) {
        ...
        if (!state) {
            ...
            state.lastLoggedPos = { x: 0, y: 0 };
            state.lastLoggedTime = 0;
            state.lastLoggedRoom = null;
            state.lastZoneName = null;
        }
        ...
        try {
            ...
            const rendered = this.renderSemanticPosition(tagMac, floor, rawPos, semantic, state.position);
            let finalPos = rendered.pos;
            let lastPosName = rendered.displayName;
            let cleanRoomName = rendered.room ? rendered.room.name : (semantic.mode === 'CORRIDOR' ? 'circulation' : null);
            let zoneType = rendered.zoneType;


            // Independent Zone Detection from Polygon
            let detectedZoneName = null;
            const pt = point([finalPos.x, finalPos.y]);
            for (const z of this.rooms) {
                if (z.type === 'zone' && z.polygonGeo && String(z.floor_id) === String(floor)) {
                    if (turf(pt, z.polygonGeo)) {
                        detectedZoneName = z.name;
                        break;
                    }
                }
            }
            state.lastZoneName = detectedZoneName;
            ...
            const HISTORY_TIME_THRES = 30000; // 30s heartbeat
            const HISTORY_DIST_THRES = 200;   // 2m movement

            const roomChanged = semantic.lastStableRoomId !== state.lastLoggedRoom;
            const distFromLastLog = Math.sqrt((finalPos.x - state.lastLoggedPos.x) ** 2 + (finalPos.y - state.lastLoggedPos.y) ** 2);
            const timeSinceLastLog = Date.now() - state.lastLoggedTime;

            const shouldLogHistory = roomChanged || distFromLastLog > HISTORY_DIST_THRES || timeSinceLastLog > HISTORY_TIME_THRES;

            if (shouldLogHistory) {
                state.lastLoggedPos = { ...finalPos };
                state.lastLoggedTime = Date.now();
                state.lastLoggedRoom = semantic.lastStableRoomId;
            }
    ...
    async snapToRoom() {
        try {
            // Fetch assets that have a room assigned
            const res = await pool.query("SELECT id, room_id, floor_id FROM assets WHERE room_id IS NOT NULL");
            const assetsToSnap = res.rows;

            for (const asset of assetsToSnap) {
                const roomObj = this.rooms.find(r => String(r.id) === String(asset.room_id) && r.type === 'room');
                if (roomObj && roomObj.anchor_x != null && roomObj.anchor_y != null) {
                    // Update in-memory assignments immediately
                    this.assetAssignments.set(asset.id.toLowerCase(), roomObj.id);

                    // Get position with deterministic jitter
                    const targetPos = this.getRoomCentroid(roomObj, asset.id);
                    const targetX = Math.round(targetPos.x);
                    const targetY = Math.round(targetPos.y);


                    // 1. Update asset location in DB
                    const savedAsset = await Asset.updateLocation(
                        asset.id,
                        roomObj.floor_id,
                        roomObj.name,    // zone
                        targetX,
                        targetY,
                        roomObj.id,      // last_room_id
                        false,           // shouldLogHistory = false
                        null             // lastPos
                    );

                    // 2. Emit position for real-time update
                    this.emit('position', {
                        id: asset.id,
                        floor: roomObj.floor_id,
                        x: targetX,
                        y: targetY,
                        zone: roomObj.name,
                        room: roomObj.name,
                        semanticMode: 'IN_ROOM',
                        semanticReason: 'startup_snap',
                        type_id: savedAsset?.type_id,
                        timestamp: new Date().toISOString(),
                        known: true
                    });

                    // 3. Force in-memory tagState update (even if it already exists)
                    this.tagState.set(asset.id, {
                        ... (this.tagState.get(asset.id) || {}),
                        floor: roomObj.floor_id,
                        ...
                        semantic: {
                            ...
                            confidence: 1.0,
                            reason: 'manual_snap',
                            lastStableRoomId: roomObj.id
                        },
                        // Reset smoothing history to the new snapped position
                        lastLoggedPos: { x: targetX, y: targetY },
                        lastLoggedTime: Date.now(),
                        lastLoggedRoom: roomObj.id,
                        _zoneHistory: [roomObj.id],
                        _flipTimestamps: []
                    });
                }
            }
        } catch (e) {
            console.error('[RTLS] Error in snapToRoom:', e.message);
        }
    }
